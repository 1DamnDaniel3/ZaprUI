...
export function RunZapret() {
    ...
    return (
        <div className={`${s.wrapper} ${batRunning ? s.running : ""}`}>
            {warning && (
                <DefaultWarning text={warning.text} type={warning.type} />
            )}
            ...
            {criticalError && (
                <BigError
                    bigError={criticalError}
                    setBigError={newCriticalError}
                />
            )}
            ...
            <span className={`${s.footer} ${batRunning ? s.running : ""}`}>
                ...
                <a
                    className={s.link}
                    onClick={() => OpenURL("https://github.com/1DamnDaniel3")}
                >
                    1DamnDaniel3
                </a>
                ,{" "}
                <a
                    className={s.link}
                    onClick={() => OpenURL("https://github.com/Saltein")}
                >
                    Saltein
                </a>
            </span>
